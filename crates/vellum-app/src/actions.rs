...
impl ActiveState {
    ...
    fn show_the_library(&mut self) {
        // The same gestures the tab swap has to close, for the same reason: going home is a
        // way for a caret or an eraser sweep to end without a release. The library screen
        // draws neither of them, so a caret left open here holds the keyboard against a board
        // that is not on screen — and it must be settled **before** the editor below is
        // swapped for a blank one, or the edit is written against the wrong board. `settle`
        // is idempotent, so this costs nothing when nothing is open.
        self.settle();
    ...
    pub(crate) fn build_demo(&mut self, name: &str) {
        match name {
            ...
            // drifted once already: fixtures that were dispatchable and named nowhere a
            // person could find them, so the only way to discover one was to read
            // `actions.rs`. A fixture nobody can find is a fixture nobody runs.
            other => log::warn!(
                "--demo: no fixture called `{other}`. The board fixtures are: shapes, empty, \
                 table, chart, mindmap, kanban, card-drag, typing, typing-shortcuts, \
                 shape-fill, caret, connector, placing, snapping, object-eraser, \
                 group-handles, locked-arrange, widget-edit, links, zoom-flicker, \
                 move-images, leave-frame, duplicate, copy-paste, context-menu, \
                 edit-then-delete, reproject-cost, paste-repeat, frame-marquee, grid-snap, \
                 readme, ports, connector-ends. And these fire a menu row: export-svg, \
                 export-pdf, export-png, present."
            ),
        }
    }
    ...
    pub(crate) fn press_like_the_window_with(
        ...
        // The same ladder, in the same order, as `App::window_event`.
        let claimed = self.editing.is_some() && self.type_key(key, text);
    ...
    pub(crate) fn choose_tool(&mut self, tool: Tool) {
        ...
        self.finish_erase();
        // A port drag borrows `input::Tool::Place`, so picking a real tool would leave the
        // arm set and turn the next placement into a connector. Cleared without restoring
        // anything, because the line below sets the input tool anyway — the sibling of the
        // line above rather than a second thought.
        self.port_arm = None;
    ...
    pub(crate) fn act_on(&mut self, intent: Intent) {
        ...
        match intent {
            ...
            Intent::PlaceSample { at } => {
                // A port or endpoint drag borrows `input::Tool::Place` while the palette's
                // tool is whatever the user left armed, so the two arms below would otherwise
                // be asked about a pen or an eraser that is not in this gesture at all.
                // Guarded by the arm rather than by the tool, which is the state that
                // actually says what the button is doing.
                if self.port_arm.is_some() || self.endpoint_arm.is_some() {
                    return;
                }
            ...
            Intent::Place { at, to } => {
                ...
                // An armed endpoint drag owns the release. `place` would otherwise be asked
                // to create whatever the palette's tool makes — which, with Select armed, is
                // nothing at all, so the gesture would simply have been swept and thrown
                // away.
                if let Some((doc, is_end)) = self.endpoint_arm.take() {
                    let tool = self.shell.tool();
                    ...
                    // endpoint arm above exists to avoid.
                    //
    ...
    fn editing_is_single_line(&self) -> bool {
        ...
        self.editor
            ...
            .is_some_and(|projected| {
                matches!(
                    projected.item.kind,
                    // A frame's title is one line by definition — it is drawn in a
                    // single-line header where a second line has nowhere to go. So Enter
                    // commits rather than inserting a break.
                    ItemKind::Frame { .. }
                )
            })
    }
    ...
    ///
    /// `input::Tool::Place` is borrowed rather than a mode of its own: the press, the
    /// samples and the release are the three events a create tool already produces,
    /// they are already tested, and
    ...
    fn rebind_connector(&mut self, doc: DocId, is_end: bool, at: WorldPoint) {
        ...
        // the direction the line reads as, so a re-attach would turn an arrow round.
        let rebound =
    ...
    /// (the pen) and 23 (the frame) a third time: state accumulated where the painter cannot
    /// see it.
    ///
    ...
    ///
    /// Answering that is what keeps the Escape ladder from spending two rungs on one press.
    ...
    pub(crate) fn cancel_drag(&mut self) -> bool {
        ...
        self.finish_erase();
        // A connector drag from one of the four ports is the same shape again, and this is
        // the *only* thing that ends one without a release: Escape, a tab switch and quitting
        // all arrive here — feedback 35's rule, applied at the moment the second borrower of
        // `Tool::Place` was written rather than after somebody reported the pointer being
        // stuck. Left armed, the next press anywhere on the board would draw a line from a
        // dot the user pressed a minute ago.
        ...
        let Some(drag) = self.drag.take() else {
            return cancelled || drawing || carrying || erasing || wiring;
        };
        for (scene, _, original) in &drag.items {
    ...
    ///
    /// The lock is honoured inside [`crate::editor::Editor::delete_selection`] rather than
    /// here, so the menu row, `Delete` and `Backspace` all get the same answer — see the
    /// known-defects entry on locking for why `Command::Delete`'s own gate is not enough.
    ...
    fn swap_in(&mut self, editor: crate::editor::Editor, camera: Option<Camera>) {
        ...
        //
        // ⚠ **`settle`, not the two of them by hand.** A tab switch is not a board mutation,
        // so `run`'s own `settle` is never reached on this path, and the list belongs in one
        // function rather than at each of its call sites. **Above the `mem::replace`**, and
        // that is load-bearing: this is feedback 27's rule — give every way a gesture can end
        // without a release a call to the thing that closes it.
        self.settle();
