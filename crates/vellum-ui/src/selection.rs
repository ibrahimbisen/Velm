...
pub enum ItemFacet {
    ...
    Connector,
    Group,
    /// A kind this panel cannot edit: a headline, a position and a size, the padlock, and
    /// no sections at all.
    ///
    /// Its own variant rather than a reused one, and that is the load-bearing half. A facet
    /// is not only a label, it is a gate — [`crate::chrome`] derives `any_group` from
    /// `facet == ItemFacet::Group` — so borrowing an existing facet for a kind that cannot
    /// act would put a live control on an item that has nothing for it to do. A variant
    /// nothing matches on cannot grow one by accident.
    ///
    /// `vellum_app::inspect::facet_of` decides which kinds land here, and carries the
...
impl ItemFacet {
    ...
    pub const fn noun(self) -> &'static str {
        match self {
            ...
            Self::Group => "Group",
            // The same word the header already uses for a selection whose kinds disagree —
            // "4 objects" — so an item the panel cannot edit is named in the vocabulary the
            // panel already has rather than in one invented for it.
            Self::Other => "Object",
        }
    }
}
...
/// English plurals for every noun in [`ItemFacet`]. None of them is irregular, so a suffix
/// rule is enough and a table would only be a place for them to drift.
...
mod tests {
    ...
    fn plurals_cover_every_facet_noun() {
        ...
        // `ItemFacet::Other`, which is what a kind the panel cannot edit reports. Pinned
        // on the **word**, not the suffix rule: `headline` lower-cases the plural, so the
        // case can never disagree, but the noun can drift. Renaming `Other` to "Item"
        // would make two of them read "2 items" beside the mixed fallback's "2 objects" —
        // two names for one idea, on the same header.
        assert_eq!(plural(ItemFacet::Other.noun()), "Objects");
    }
}
